...
\vspace{0.25 cm}\normalsize\sf
\uncover<5->{Recursive counters can be expanded into offsets (cumulative sum).}
\end{frame}
...
\vspace{0.35 cm}
\uncover<2->{Recursive counters can be expanded into these, too.}
\end{frame}
...
\begin{tabular}{p{0.25\linewidth} p{0.3\linewidth} p{0.33\linewidth}}
                   & {\bf recursive types?} & {\bf other features} \\
\textcolor{darkblue}{Recursive counters} & yes, by terminating list sizes with $-1$ & efficient iteration \\
\textcolor{darkblue}{Parquet style} & \mbox{yes, by not capping} \mbox{repetition level} \mbox{(though Parquet doesn't} \mbox{support recursive types)} & \mbox{tightly packed without} compression \\
